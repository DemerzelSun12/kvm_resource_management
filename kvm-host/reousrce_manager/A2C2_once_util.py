...
class Env():
    def __init__(self,vm_num):
        ...
        self.vm_num = 16                  #虚拟机数量

        self.vm_curr_mem = [1300] * self.vm_num       #保存内存的数量的标志
        self.vm_curr_cpu = [2] * self.vm_num         #当前cpu数量
        self.vm_curr_bandwith  = [4] * self.vm_num   #初始带宽为2M

        self.vm_sum_mem = 1300*self.vm_num             #所以虚拟机所有的内存大小
        self.vm_sum_cpu = 8*self.vm_num               #所有可以分配的VCPU总和,不会使用到
        ...
        self.vm_max_mem = 5120            #VM最大mem大小
        self.vm_min_mem = 400
        self.vm_max_bandwith = 50

        self.vm_max_cpu = 8               #最大的cpu数量
        ...
        self.netcards = {1: "vnet28", 2: "vnet16", 3: "vnet17", 4: "vnet14", 5: "vnet18", 6: "vnet19", 7: "vnet29", 8: "vnet20", 9: "vnet21", 10: "vnet22", 11: "vnet23", 12: "vnet24", 13: "vnet25", 14: "vnet26", 15: "vnet27"}
    ...
    def execute_bandwith(self,op,vm_id,percent):
        ...
        if op==1:
            ...
            os.system("virsh domiftune {} {} --live --config --inbound {},{},{} > /dev/null".format(vm_domain,mac,round(newband_size*128),round(newband_size*128),round(newband_size*128)))
            os.system("virsh domiftune {} {} --live --config --outbound {},{},{} > /dev/null".format(vm_domain,mac,round(newband_size*128),round(newband_size*128),round(newband_size*128)))
            ...
            if newband_size > self.vm_min_bandwith:
                os.system(
                    "virsh domiftune {} {} --live --config --inbound {},{},{} > /dev/null".format(vm_domain,mac,round(newband_size*128),round(newband_size*128),round(newband_size*128)))
                os.system(
                    "virsh domiftune {} {} --live --config --outbound {},{},{} > /dev/null".format(vm_domain,mac,round(newband_size*128),round(newband_size*128),round(newband_size*128)))
                self.vm_curr_bandwith[vm_id] = newband_size
                print("VM {} DOWN BANDWIDTH {} TO {}".format(vm_id + 1, band_size, self.vm_curr_bandwith[vm_id]),flush=True)
        else:
            print("Op Error!!")
    ...
    def execute_mem(self,op,vm_id,percent):
        ...
        #如果为减少,则说明该值为负数,那么我们就按照比例进行减小即可
            #print("vm id:{}; mem_size:{}MB ; New mem_size:{}MB".format(vm_id,mem_size,newmem_size))
            print("VM{} DOWN MEM {}MB To {}MB".format(vm_id+1, mem_size,round(abs(mem_size*(-percent)))),flush=True)
            val = os.system("virsh setmem {} --size {}MB > /dev/null".format(vm_domain,round(abs(mem_size*(-percent)))))
            self.vm_down_mem += (self.vm_curr_mem[vm_id]-round(abs(mem_size*(-percent))))  #round(mem_size*abs(1+percent))
        ...
        #做内存更正用的
            ...
            for i in self.vm_domain.items(): #遍历每一个虚拟机
                if self.vm_curr_mem[i[0]-1]+round(self.correct_mem/self.vm_num) < self.vm_max_mem:
                    ...
                    self.over_max_correct_mem += self.vm_curr_mem[i[0]-1]+round(self.correct_mem/self.vm_num)-self.vm_max_mem
    ...
    def checkHealthy(self,vm_id=0):
        if(self.observations[vm_id][0]<0.85 and self.observations[vm_id][2]<0.8 and self.observations[vm_id][4]<0.8):
            return True
        else:
            return False
    ...
    def steps(self,actions,mode="Train"):
        ...
        for i in range(self.vm_num):
            ...
            if action_cpu[i]<0 and self.observations[i][0]<=0.75 and self.vm_curr_cpu[i]-1 >=1:
            ...
            if action_bandwith[i]<0 and self.observations[i][4]<= 0.75 and self.vm_curr_bandwith[i]*(-action_bandwith[i]) >= self.vm_min_bandwith:
            ...
            elif action_bandwith[i]>0 and self.observations[i][4]>0.75 and self.vm_curr_bandwith[i]*(1+action_bandwith[i])< self.vm_sum_bandwith:
                pass
            else:
                pass
        ...
        for i in range(self.vm_num):
            ...
            if action_cpu[i]>0 and self.observations[i][0]>0.65 and self.vm_curr_cpu[i]+1 <= self.vm_max_cpu:
                ...
                #print("++++ VM:{} UP CPU {} cpu_usage:{}% ++++".format(i+1,self.vm_curr_cpu[i],self.observations[i][0]),flush=True)
            elif action_cpu[i] < 0 and self.observations[i][0]<0.75 and self.vm_curr_cpu[i]>=1:
                pass
            elif action_cpu[i] == 0 and self.observations[i][0]<0.75 and self.observations[i][0]>0.5:
                pass
            else:
                bad_action_cpu[i] = 1
                reword_cpu[i] -= 1
            ...
            if action_cpu[i]>0 and self.observations[i][4]>0.75 and self.vm_curr_bandwith[i]*(1+action_bandwith[i]) < self.vm_max_bandwith:
            ...
            elif action_bandwith[i] < 0 and self.observations[i][4]<0.75 and self.vm_curr_bandwith[i] >= self.vm_min_bandwith:
                pass
        ...
        for i in range(self.vm_num):
            #首先计算内存
            if self.pre_observations[i][2] > 0.95 or self.pre_observations[i][0]>0.86:
            ...
            if self.pre_observations[i][2] >= self.observations[i][2] and self.pre_observations[i][2] > 0.8:
                reword_mem[i] += 8 #这是个优秀的操作

            #2.mem利用率下降,且超过合理范围
            elif self.pre_observations[i][2] >= self.observations[i][2] and self.observations[i][2] < 0.65:
                reword_mem[i] += -4
            ...
            if self.observations[i][4] > 0.9 and self.vm_curr_bandwith[i] <self.vm_max_bandwith:
                self.bad_state_vm[i] = 1
                reword_bandwith[i] += -8  # 我们的目标就是调节到 bad_state_vm为0 即可了
        ...
        print("+++ Reword:{},Sum_mem:[{}/{}],Sum cpus:{},Sum bandwidth:{} +++".format(sum(reword_bandwith_sum+reword_mem_sum+reword_cpu_sum),sum(self.vm_curr_mem),self.vm_sum_mem,sum(self.vm_curr_cpu),sum(self.vm_curr_bandwith)))
        ...
        if mode=="Train":
            ...
            return np.array(self.vm_state),np.array(reword_sum),1
        else:
            return np.array(self.vm_state), np.array(reword_sum),0
