...
class Env():
    def __init__(self, vm_num):
        ...
        self.vm_curr_mem = [1500] * self.vm_num  # 保存内存的数量的标志
        self.vm_curr_cpu = [2] * self.vm_num  # 当前cpu数量
        self.vm_curr_bandwith = [4] * self.vm_num  # 初始带宽为2M

        self.vm_sum_mem = 1500 * self.vm_num  # 所以虚拟机所有的内存大小
        ...
        # 虚拟机的domain名
        self.vm_domain = {1: "ubuntu18.04-05", 2: "ubuntu18.04-06", 3: "ubuntu18.04-08", 4: "ubuntu18.04-09",
                          5: "ubuntu18.04-10",
                          6: "ubuntu18.04-11", 7: "ubuntu18.04-12", 8: "ubuntu18.04-13", 9: "ubuntu18.04-14",
                          10: "ubuntu18.04-15",
                          11: "ubuntu18.04-16", 12: "ubuntu18.04-17", 13: "ubuntu18.04-18", 14: "ubuntu18.04-19",
                          15: "ubuntu18.04-20"}

        self.ip = {1: "172.17.0.3", 2: "172.17.0.4", 3: "172.17.0.6", 4: "172.17.0.7",
                   5: "172.17.0.8",
                   6: "172.17.0.9", 7: "172.17.0.10", 8: "172.17.0.11", 9: "172.17.0.12",
                   10: "172.17.0.13",
                   11: "172.17.0.14", 12: "172.17.0.15", 13: "172.17.0.16", 14: "172.17.0.17",
                   15: "172.17.0.18"}
    ...
    '''
    def gatherDockerStatus(self):
        mem_usage = []
        cpu_usage = []
        bandwith_usage = []
        statuess = statues = subprocess_popen("docker stats --no-stream | sed '1d' |sed '1!G;h;$!d'| sed '1d'")
        for line in statuess:
            statues = line.replace("%","").replace("/","").replace("MiB","").replace("GiB","").replace("kB","").replace("MB","").replace("GB","").split(" ")
            tmp_list = []
            for i in range(len(statues)):
                if (statues[i] != ''):
                    tmp_list.append(statues[i])
            mem_usage.append(float(tmp_list[5])/100)
            cpu_usage.append(float(tmp_list[2])/100)
            bandin = float(tmp_list[6][0:-1])
            bandout = float(tmp_list[7][0:-1])
            bandwith_usage.append(np.random.rand())
        return mem_usage,cpu_usage,bandwith_usage
    ...
    def gather(self):
        ...
        # bandwith_nums = self.vm_curr_bandwith
        t1 = time.time()
        mem_usage,cpu_usage,bandwith_usage = self.gatherDockerStatus()
        print("gather time is {}".format(t1-time.time()))
        # 数据汇总以及归一化.
        observation = [[cpu_usage[i]/100, cpu_nums[i] / self.vm_max_cpu, mem_usage[i]/100, mem_nums[i] / self.vm_max_mem,
                        bandwith_usage[i]/bandwith_nums[i], bandwith_nums[i] / self.vm_max_bandwith] for i in range(self.vm_num)]
    ...
    def execute_cpu(self, op, vm_id):
        ...
        if op == 1 and cpu_num < self.vm_max_cpu:
            try:
                ret = os.system("docker update --cpus {} {} > /dev/null".format(cpu_num + 1,vm_domain))
            except:
                ret = -1
    ...
    def execute_bandwith(self, op, vm_id, percent):
        ...
        if op == 1:
            ...
            os.system('docker exec {} bash -c "tc class change dev eth0 parent 1: classid 1:21 htb rate {}Mbit ceil {}Mbit" &'.format(vm_domain,
                                                                                                    round(
                                                                                                        newband_size),
                                                                                                    round(
                                                                                                        newband_size)))
            ...
            if newband_size > self.vm_min_bandwith:
                os.system(
                    'docker exec {} bash -c "tc class change dev eth0 parent 1: classid 1:21 htb rate {}Mbit ceil {}Mbit" &'.format(vm_domain,round(
                        newband_size), round(newband_size)))
                self.vm_curr_bandwith[vm_id] = newband_size
    ...
    def execute_mem(self, op, vm_id, percent):
        ...
        if op == 1 and newmem_size <= self.vm_max_mem and self.vm_up_count != 0:
            ...
            val = os.system("docker update --memory {}m --memory-swap -1 {} > /dev/null".format(newmem_size,vm_domain))
            ...
            val = os.system("docker update --memory {}m --memory-swap -1 {} > /dev/null".format(self.vm_max_mem,vm_domain))
            ...
            print("VM{} DOWN MEM {}MB To {}MB".format(vm_id + 1, mem_size, round(abs(mem_size * (-percent)))),
                  flush=True)
            val = os.system(
                "docker update --memory {}m --memory-swap -1 {} > /dev/null".format(round(abs(mem_size * (-percent))),vm_domain))
            self.vm_down_mem += (
        ...
        # 没有人可以增加时,就没有机器去获取,所以需要修正
            ...
            for i in self.vm_domain.items():
                ...
                os.system("docker update --memory {}m --memory-swap -1 {} > /dev/null".format(self.vm_curr_mem[i[0] - 1],i[1]))
            diff = 0
            self.correct_mem = 0
    ...
    def steps(self, actions, mode="Train"):
        ...
        for i in range(self.vm_num):
            ...
            # print("---- VM:{} DOWN MEM {} mem usage:{} ----".format(i + 1,
            #                                                            self.vm_curr_mem[i] * (1 + action_mem[i]),
            #                                                            self.observations[i][2]),flush=True)
            ...
            # print("---- VM:{} DOWN CPU {} cpu usage:{}% ----".format(i+1,self.vm_curr_cpu[i],
            #                                                       self.observations[i][0]),flush=True)
        ...
        for i in range(self.vm_num):
            ...
            if self.observations[i][4] > 0.9 and self.vm_curr_bandwith[i] < self.vm_max_bandwith:
                self.bad_state_vm[i] = 1
                reword_bandwith[i] += -10  # 我们的目标就是调节到 bad_state_vm为0 即可了
        ...
        print("+++ Now Bad VM num is [{}/{}] +++".format(self.bad_state_vm_pre, self.vm_num))
        print("-----------------------------------------------------------")
        time.sleep(1)
